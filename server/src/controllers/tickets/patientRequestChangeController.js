...
export const getTicketByID = async (req, res) => {
  try {
    const ticket = await PatientRequestTicket.findById({ _id: req.params.id });
...
export const getInProgressTicketsByOpsId = async (req, res) => {
  try {
    ...
    const tickets = await PatientRequestTicket.find({ responsibleMember: opsId, status:"In Progress" });
...
export const getAllTicketsByOpsId = async (req, res) => {
  try {
    const opsId = new mongoose.Types.ObjectId(req.params.opsId);
    const tickets = await PatientRequestTicket.find({ responsibleMember: opsId });
    if (!tickets) return res.status(404).json({ error: "Tickets not found" });
    return res.json(tickets);
  } catch (err) {
    return res.status(500).json({ error: err.message });
